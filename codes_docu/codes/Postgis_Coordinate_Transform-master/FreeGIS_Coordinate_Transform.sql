...
CREATE OR REPLACE FUNCTION FreeGIS_Coordinate_Transform(
	in schema_name text,--schema name of input table,e.g. 'public'
	in table_name text,--name of input table,e.g. 'test'
	in transform_type FreeGIS_coordinate_transform_type--transform type, e.g. 'WGS2GCJ'
) RETURNS text As
$BODY$
DECLARE
	server_version int;
...
BEGIN
	...
	if(rec is null) then
		raise notice 'Table does not exist, maybe schema_name or table_name is wrong!';
		return 'failed';
	end if;
	...
	if(rec is null) then
		raise notice 'only support geometry!';
		return 'failed';
	end if;
	
	--
	if(rec.coord_dimension!=2) then
		raise notice 'only support two dimensional features!';
		return 'failed';
	end if;
	...
	if(transform_type!='BDMKT2BDWGS' and transform_type!='BDMKT2WGS') then
		if(rec.srid!=4326) then
			raise notice 'only support WGS84(EPSG:4326)!';
			raise notice 'please transform to 4326 and then use this script';
			return 'failed';
		end if;
	else
		--
		if(rec.srid!=3857) then
			raise notice 'only support (EPSG:3857)';
			return 'failed';
		end if;
	end if;
	...
	geom_type!='POLYGON' and geom_type!='MULTIPOLYGON') then
		raise notice 'only support Point,LineString,Polygon,MultiPoint,MultiLineString,MultiPolygon';
		return 'failed';
	end if;
	...
	case geom_type
		...
		when 'MULTIPOLYGON' then
			...
			raise notice 'do not support this type';
			return 'failed';
	end case;	
	raise notice 'finish!';	
	return 'success';
